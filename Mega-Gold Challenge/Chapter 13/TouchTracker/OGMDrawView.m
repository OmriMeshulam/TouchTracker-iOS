...
@property (nonatomic, strong) UIMenuController *colorMenu;
...
#pragma mark - Color Menu Options
-(void)colorMenu:(UIGestureRecognizer *)gr
{
    ...
    [self becomeFirstResponder];
    self.colorMenu = [UIMenuController sharedMenuController];
    ...
    self.colorMenu.menuItems = @[redColor,blueColor];
    
    [self.colorMenu setTargetRect:CGRectMake(point.x, point.y, 2, 2) inView:self];
    [self.colorMenu setMenuVisible:YES];
}
